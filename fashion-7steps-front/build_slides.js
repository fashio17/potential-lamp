/**
 * 無料ミニ講座（全3本）動画用スライド 生成スクリプト
 *
 *   node build_slides.js
 *
 * 用途：Instagram → LINE 登録者に配信する動画3本の画面
 * 方針：スマホ視聴前提。1枚1メッセージ、本文は最小18pt。
 * 出力：無料ミニ講座スライド.pptx （全23枚 / 16:9 ワイド）
 *        動画1 = 1-7 ／ 動画2 = 8-15 ／ 動画3 = 16-23
 */
...
pres.title = "無料ミニ講座 ― 選ばれるファッションのお仕事";
...
const W = 13.333;
const M = 0.9;              // 動画用に少し広め
const CW = W - M * 2;       // 11.533

/* ---------- ヘルパー ---------- */

function chapter(s, n, dark) {
  s.addText(`動画 ${n}`, {
    x: W - M - 2.0, y: 0.34, w: 2.0, h: 0.32, align: "right", valign: "middle",
    fontFace: BODY, fontSize: 11, color: dark ? ROSE_LT : MUTED, charSpacing: 2, margin: 0,
  });
}

/** 動画の表紙（ダーク・大きい文字） */
function coverSlide(n, kicker, title, sub, notes) {
...
/** 明るいコンテンツスライド */
function contentSlide(n, title, lead) {
  const s = pres.addSlide();
  chapter(s, n, false);
  s.addText(title, {
...
/** ダークな一言スライド */
function statementSlide(n, big, sub, notes) {
  const s = pres.addSlide();
  s.background = { color: PLUM };
  chapter(s, n, true);
  s.addText(big, {
...
/** 次回予告 */
function nextSlide(n, label, title, notes) {
  const s = pres.addSlide();
  s.background = { color: PLUM };
  chapter(s, n, true);
  s.addText(label, {
...
coverSlide(1, "パーソナルスタイリスト／イメージコンサルタントの方へ",
  "資格を取ったのに、\nなぜ選ばれないのか",
  "約10分",
  "冒頭15秒が勝負。名乗りより先にタイトルを言ってもいい。テンポ優先。");

{
  const s = contentSlide(1, "こんなこと、ありませんか", "ひとつでも当てはまったら、最後までご覧ください");
  const items = [
...
{
  const s = contentSlide(1, "「診断ができます」では、もう選ばれない", "資格を持っている方が、本当に増えました");
...
statementSlide(1,
  ...
  "動画1の山。自分の失敗として語れると強い。実体験で。");

{
  const s = contentSlide(1, "これは、意志の問題ではありません", "あなたの説明が下手だからでもありません");
  ...
  pts.forEach((t, i) => {
    ...
    numCircle(s, M + 0.28, y + 0.14, 0.44, i + 1, PLUM, ROSE_LT, 15);
    s.addText(t, {
      x: M + 0.95, y, w: CW - 1.3, h: 0.72,
      fontFace: BODY, fontSize: 18, color: INK, margin: 0, valign: "middle",
    });
  });
  s.addNotes("次回への引き。理由そのものはここでは言わない。");
}

nextSlide(1, "次回", "お客様が変わらない、\n本当の理由",
  "「たぶん聞いたことがない話です」と添えると視聴率が上がる。");

/* =========================================================
   動画2（スライド 8 - 15）
   ========================================================= */

coverSlide(2, "無料ミニ講座 2本目",
  "お客様が変わらない、\n本当の理由",
  "約12分　／　3本の中で、いちばんお伝えしたい回です",
  "3本の中心。ここで「この人は他と違う」と思ってもらう。");

{
  const s = contentSlide(2, "買おうとした瞬間、心の中で聞こえる声", "お客様の頭の中で、何が起きているか");
  const voices = [
    "「私なんかが、こんな明るい色を着ても」",
    "「あ、けっこう高いな。安いほうでいいや」",
    "「これ着てたら、家族に何か言われそう」",
...
{
  const s = contentSlide(2, "この声を「観念」といいます", "ファッションに対する、その人の思い込みです");
  ...
  const habits = ["「安かったから、つい買っちゃって」", "「素敵な服を着てないと、恥ずかしい」", "「結局、好きなものを着ちゃうよね」"];
  habits.forEach((t, i) => {
    const y = 2.68 + i * 0.72;
    s.addShape(pres.ShapeType.roundRect, {
  ...
  const past = ["子どもの頃、母が選んだ服をずっと着ていた", "背が高くて、可愛い服が似合わなかった", "「変な格好だと一緒に歩けない」と言われた"];
  past.forEach((t, i) => {
    const y = 2.68 + i * 0.72;
    s.addShape(pres.ShapeType.roundRect, {
...
statementSlide(2,
  ...
  "動画2の結論。ゆっくり。ここだけは絶対に早口にしない。");

{
  const s = contentSlide(2, "観念は、3つの順番で変わります", null);
  const phases = [
  ...
  phases.forEach((p, i) => {
    ...
    numCircle(s, x + cw / 2 - 0.32, 2.45, 0.64, i + 1, PLUM, ROSE_LT, 21);
    s.addText(p[0], {
...
{
  const s = contentSlide(2, "「似合う」と「なりたい」は、ズレます", null);
...
{
  const s = contentSlide(2, "順番を変えるだけで、変わります", "同じ時間、同じ資格、同じ診断結果です");
  ...
  s.addNotes("動画2のまとめ。「順番を変えるだけ」を強調する。");
}

nextSlide(2, "次回・最終回", "選ばれている人が、\n整えている7つ",
...
coverSlide(3, "無料ミニ講座 3本目（最終回）",
  "選ばれている人が、\n整えている7つ",
  "約12分　／　やることリストではなく、現在地を知るための地図です",
  "地図を渡して現在地を自覚してもらう。「7つもやるのか」と思わせないこと。");

{
  const s = contentSlide(3, "選ばれている方が、整えている7つ", "今日は覚えなくて大丈夫です。地図として見てください");
  ...
  steps.forEach((st, i) => {
    ...
    numCircle(s, M + 0.16, y + 0.09, 0.36, i + 1, PLUM, ROSE_LT, 12);
    s.addText(st[0], {
...
{
  const s = contentSlide(3, "あなたは、どこが空いていますか", "4つだけ。心の中で、はい・いいえで答えてみてください");
  const qs = [
  ...
  qs.forEach((t, i) => {
    ...
    numCircle(s, M + 0.24, y + 0.14, 0.5, i + 1, PLUM, ROSE_LT, 17);
    s.addText(t, {
...
{
  const s = contentSlide(3, "個別相談で、一緒に整理します", "あなたの現在地を、一緒に言葉にしていく時間です");
...
{
  const s = contentSlide(3, "こんな方に、お会いしたいです", null);

  const yes = [
    "資格は取ったけれど、そこから先が分からない方",
    "単発の診断で、少し疲れてきてしまった方",
    "お客様に、もっと深いところで役に立ちたい方",
  ];
  yes.forEach((t, i) => {
    const y = 2.2 + i * 0.85;
    s.addShape(pres.ShapeType.roundRect, {
...

{
  ...
  s.addShape(pres.ShapeType.ellipse, { x: 9.1, y: 1.9, w: 3.3, h: 3.3, fill: { color: PLUM_DK } });
...
pres.writeFile({ fileName: "無料ミニ講座スライド.pptx" }).then((f) => {
