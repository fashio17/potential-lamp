...
#   ④「服だけの話ではありませんで切れたら、なんの話か分かりません」（9/19・2度目のご指摘）
#      → **着地がなかった。**「服だけの話ではない」と言ったまま、何の話かを言っていない。
#      **11・12枚目を足して、見ている人の商品に着地させた。**
#      12枚24.8秒。**最後は問いかけで終わる**（プロフィールに行く理由をつくるため）。
...
# 9 中景・建物の前
cutc "$TATE"  8.60 1.28 "1080:1920:0:0"  $SP/segR19/k9.mp4
# 10 引き・並木。「服だけの話ではありません」で画をいったん広げる
cutc "$NAMI"  5.20 1.28 "1080:1920:0:0"  $SP/segR19/k10.mp4
# 11 建物の前を、まっすぐ歩いて近づく。**ここから見ている人に向き直る**
#    （はじめ t=9.00 を使ったが、横を向いていて「あなたの」に合わなかった。9/19に変更）
#    t=14.68 で終わるので、手が出る t≧15 には入らない
cutc "$TATE" 13.40 1.28 "1080:1920:0:0"  $SP/segR19/k11.mp4
# 12 締め：歩いて近づき、中景で止まる。**指は入らない**
#    最後の問いかけ（誰に向けていますか）を、いちばん近い画で置く
cutc "$NAMI" 17.60 1.28 "1080:1920:0:0"  $SP/segR19/k12.mp4
